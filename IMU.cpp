...
void Kalman::iteration() {
  ...
  int32_t X_degrees = -E_X_degrees();
  int32_t Y_degrees = E_Y_degrees();
  double X = X_degrees > 0 ? X_degrees : 360 + X_degrees;
  double Y = Y_degrees > 0 ? Y_degrees : 360 + Y_degrees;
  double X_radians = X / 180 * M_PI;
  double Y_radians = Y / 180 * M_PI;
  X_corrected = cos(X_radians) * _magnetometer.X() + sin(X_radians) * sin(Y_radians) * _magnetometer.Y() + sin(X_radians) * cos(Y_radians) * _magnetometer.Z();
  Y_corrected = cos(Y_radians) * _magnetometer.Y() - sin(Y_radians) * _magnetometer.Z();
  Z_corrected = -sin(X_radians) * _magnetometer.X() + cos(X_radians) * sin(Y_radians) * _magnetometer.Y() + cos(X_radians) * cos(Y_radians) * _magnetometer.Z();
